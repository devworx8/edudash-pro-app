...
import { COMPOSER_FLOAT_GAP, COMPOSER_OVERLAY_MIN_HEIGHT, COMPOSER_ANDROID_NAV_LIFT, getBottomThinkingLabel } from './utils';
...
export const DashAssistant: React.FC<DashAssistantProps> = ({
  conversationId, onClose, initialMessage, handoffSource, uiMode,
  disableTts = false, disableQuickChips = false,
  tutorMode: externalTutorMode, tutorConfig,
}: DashAssistantProps) => {
  ...
  const canRunAssignmentsTool = useMemo(() => {
    ...
    return ['parent', 'student', 'learner', 'teacher', 'principal', 'principal_admin', 'admin', 'super_admin', 'superadmin'].includes(normalizedRole);
  ...
  const singleThinkingIndicatorEnabled = useMemo(() => getFeatureFlagsSync().dash_single_thinking_indicator_v1 !== false, []);
  ...
  useEffect(() => {
    ...
    const showSub = Keyboard.addListener(showEvent, (e) => { setKeyboardVisible(true); setKeyboardHeight(e?.endCoordinates?.height || 0); });
  ...
  const speech = useSpeechControls({ isSpeaking, speakingMessageId, speechChunkProgress, messages, speakResponse, stopSpeaking });

  const scanner = useScannerFlow({
    autoScanUserId, tierRef, messages, isLoading, isUploading, selectedAttachments,
    addAttachments, sendMessage, setInputText,
  });
  ...
  useEffect(() => {
    ...
    if (isTypingActive && !duplicateThinkingLoggedRef.current) {
      ...
      trackDashTelemetry(DASH_TELEMETRY_EVENTS.DUPLICATE_THINKING_INDICATOR_BLOCKED, { source: 'message_footer' });
    }
  ...
  const shellSubtitle = useMinimalNextGenLayout ? 'Your AI assistant'
    : isTutorUiActive ? 'Tutor Session Active'
    : uiMode === 'advisor' ? 'Advisor Mode'
    : uiMode === 'exam' ? 'Exam Builder Mode'
    : uiMode === 'orb' ? 'Orb Companion Mode'
    : 'Your AI assistant';

  const handleNewChat = useCallback(async () => { await stopAllActivity(); await startNewConversation(); }, [startNewConversation, stopAllActivity]);
  ...
  const handleOpenOrb = useCallback(() => { router.push('/screens/dash-voice?mode=orb'); }, []);
  const handleRunScheduleTool = useCallback(() => { void runTool('get_schedule', { start_date: 'today', days: 7 }); }, [runTool]);
  const handleRunAssignmentsTool = useCallback(() => { void runTool('get_assignments', { status: 'pending', days_ahead: 14 }); }, [runTool]);

  const toolActivityLabel = activeToolLabel || (hasActiveToolExecution ? 'Using tools to prepare your answer' : null);
  const showBottomThinkingDock = singleThinkingIndicatorEnabled && !streamingMessageId && !isRecording && (isTypingActive || hasActiveToolExecution);
  ...
  useEffect(() => {
    if (keyboardVisible && messages.length > 0 && isNearBottom) {
      const timer = setTimeout(() => scrollToBottom({ animated: true, delay: 50 }), Platform.OS === 'android' ? 150 : 50);
      return () => clearTimeout(timer);
    }
  ...
  const composerBottomInset = Platform.OS === 'ios' ? insets.bottom : Math.max(insets.bottom, COMPOSER_ANDROID_NAV_LIFT);
  ...
  const messageViewportInset = keyboardHeight + COMPOSER_FLOAT_GAP + composerExtraBottom + safeComposerHeight + bottomThinkingDockClearance;
  const backgroundBase: [string, string, string] = isDark ? ['#0B1020', '#0F172A', theme.background] : ['#F7FAFF', '#EEF2FF', '#F8FAFC'];
  const glowA: [string, string, string] = isDark ? ['rgba(14,165,233,0.32)', 'rgba(59,130,246,0.05)', 'transparent'] : ['rgba(14,165,233,0.35)', 'rgba(34,211,238,0.12)', 'transparent'];
  const glowB: [string, string, string] = isDark ? ['rgba(16,185,129,0.25)', 'rgba(99,102,241,0.06)', 'transparent'] : ['rgba(16,185,129,0.3)', 'rgba(59,130,246,0.08)', 'transparent'];

  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: theme.background }} edges={['top']}>
      <KeyboardAvoidingView style={[layoutStyles.container, { backgroundColor: theme.background }]} behavior={keyboardBehavior} keyboardVerticalOffset={keyboardOffset}>
        ...
        <View style={layoutStyles.contentLayer}>
          ...
          {initTimedOut && !isInitialized && (
            <View style={{ padding: 12, marginHorizontal: 16, marginTop: 8, backgroundColor: theme.surfaceVariant + 'CC', borderRadius: 12, borderWidth: 1, borderColor: theme.border }}>
              <Text style={{ color: theme.textSecondary, fontSize: 14 }}>Having trouble connecting. Try sending a message and I&apos;ll do my best to help.</Text>
            </View>
          )}
          ...
          <DashAssistantHeader
            ...
            onSelectModel={setSelectedModel} onStopAllActivity={() => void stopAllActivity()}
            onOpenOptions={() => openOptionsSheet()} onOpenOrb={handleOpenOrb}
            onClose={onClose} onClosePress={async () => { await stopSpeaking(); dashInstance?.cleanup?.(); onClose?.(); }}
          />

          <View style={[layoutStyles.messagesClip, { marginBottom: messageViewportInset }]}>
            <DashAssistantMessages
              flashListRef={flashListRef} messages={messages} renderMessage={renderMessage}
          ...
          {!isNearBottom && messages.length > 0 && (
            <TouchableOpacity
              style={[messageStyles.scrollToBottomFab, { backgroundColor: theme.primary, bottom: messageViewportInset + 12, zIndex: 220, elevation: 16 }]}
              onPress={() => { setUnreadCount(0); scrollToBottom({ animated: true, delay: 0, force: true }); requestAnimationFrame(() => scrollToBottom({ animated: false, delay: 0, force: true })); }}
              accessibilityLabel="Jump to bottom" activeOpacity={0.8} hitSlop={{ top: 12, right: 12, bottom: 12, left: 12 }}
            >
              ...
              {unreadCount > 0 && (
                <View style={[messageStyles.scrollToBottomBadge, { backgroundColor: theme.error }]}>
                  <Text style={[messageStyles.scrollToBottomBadgeText, { color: theme.onError || '#fff' }]}>{unreadCount > 99 ? '99+' : unreadCount}</Text>
                </View>
              )}
            </TouchableOpacity>
          )}
          ...
          <DashAssistantThinkingDock
            theme={theme} show={showBottomThinkingDock} label={bottomThinkingLabel}
            toolActivityLabel={toolActivityLabel} keyboardHeight={keyboardHeight}
            safeComposerHeight={safeComposerHeight} composerExtraBottom={composerExtraBottom}
          />

          <View
            style={[layoutStyles.composerArea, { bottom: keyboardHeight + COMPOSER_FLOAT_GAP + composerExtraBottom, paddingBottom: keyboardUp ? 0 : composerBottomInset }]}
            pointerEvents="box-none" onLayout={(e) => setComposerHeight(e.nativeEvent.layout.height)}
          >
            <DashInputBar
              inputRef={inputRef} inputText={inputText} setInputText={setInputText}
              enterToSend={enterToSend} selectedAttachments={selectedAttachments}
              attachmentProgress={attachmentProgress} isLoading={isLoading} isUploading={isUploading}
              isRecording={isRecording} recordingVoiceActivity={recordingVoiceActivity} isSpeaking={isSpeaking}
              partialTranscript={partialTranscript} voiceAutoSendCountdownActive={voiceAutoSendCountdownActive}
              voiceAutoSendCountdownMs={voiceAutoSendCountdownMs} placeholder="Message Dash..."
              messages={messages} onSend={() => sendMessage()} onMicPress={handleInputMicPress}
              onCancelVoiceAutoSend={cancelVoiceAutoSend} onInterrupt={stopAllActivity}
              onTakePhoto={scanner.openScanner} onAttachFile={openAttachmentSheet}
              onRemoveAttachment={handleRemoveAttachment} onQuickAction={(text) => sendMessage(text)}
              onCancel={cancelGeneration} bottomInset={0} hideQuickChips={disableQuickChips || true}
              onInputFocus={handleComposerFocus} onPasteImage={handlePasteImage}
            />
          </View>

          {alertState.bannerMode ? (
            alertState.visible && (
              <View style={{ position: 'absolute', bottom: 100, left: 16, right: 16, backgroundColor: theme.surface, borderRadius: 12, paddingHorizontal: 16, paddingVertical: 12, flexDirection: 'row', alignItems: 'center', gap: 10, shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.18, shadowRadius: 10, elevation: 8, borderWidth: 1, borderColor: theme.border, zIndex: 999 }}>
                {alertState.icon && <Ionicons name={alertState.icon as any} size={20} color={theme.primary} />}
                <Text style={{ flex: 1, color: theme.text, fontSize: 13, fontWeight: '500' }}>{alertState.message}</Text>
                <TouchableOpacity onPress={hideAlert} hitSlop={8}><Ionicons name="close" size={18} color={theme.textSecondary} /></TouchableOpacity>
              </View>
            )
          ) : (
            <AlertModal visible={alertState.visible} title={alertState.title} message={alertState.message} type={alertState.type} icon={alertState.icon as any} buttons={alertState.buttons} onClose={hideAlert} />
          )}

          <AttachmentOptionsSheet
            visible={attachmentSheetVisible} onClose={() => setAttachmentSheetVisible(false)}
            onTakePhoto={scanner.openScanner} onPickImages={() => void handlePickImages()}
            onPickDocuments={() => void handlePickDocuments()} showDocuments isBusy={isLoading || isUploading}
          />
          <DashOptionsSheet
            visible={optionsSheetVisible} onClose={() => setOptionsSheetVisible(false)}
            onNewChat={handleNewChat} onOpenHistory={() => router.push('/screens/dash-conversations-history')}
            onOpenSearch={() => router.push('/screens/app-search?scope=dash&q=dash')}
            onOpenOrb={handleOpenOrb} onOpenScanner={scanner.openScanner}
          ...
          />
          <HomeworkScanner
            visible={scanner.scannerVisible} onClose={scanner.closeScanner}
            onScanned={scanner.handleScannerScanned} title="Scan Image"
            tier={tier || 'free'} remainingScans={scanner.remainingScans} userId={autoScanUserId}
          />
        </View>
