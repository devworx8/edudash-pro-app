...
export const DashInputBar: React.FC<DashInputBarProps> = ({
      ...
      if (Platform.OS === 'web') {
        ...
        const nextHeight =
          lineCount <= 1
            ...
            : Math.min(
                FULL_CHAT_COMPACT_HEIGHT + (lineCount - 1) * FULL_CHAT_LINE_HEIGHT,
                FULL_CHAT_MAX_HEIGHT,
              );
              ...
              >
                ...
                {isImage && imageUri ? (
                  <View style={styles.attachmentImageWrapper}>
                    ...
                    {status === 'uploaded' && (
                      <View
                        style={[styles.attachmentImageBadge, { backgroundColor: theme.success }]}
                      >
                    ...
                    {/* File size label */}
                    <View
                      style={[styles.attachmentImageSize, { backgroundColor: 'rgba(0,0,0,0.7)' }]}
                    >
  ...
  const normalizeTranscript = (value: string) => value.trim().replace(/\s+/g, ' ').toLowerCase();
  ...
  const statusToneColor = isRecording
    ? theme.error
    : isLoading
      ? theme.primary
      : theme.textSecondary;
  ...
  const canShowQuickChips =
    !hideQuickChips && !hasContent && !isRecording && !isLoading && !hasMessages;
  ...
  const quickChips = [
    {
      ...
      label: 'Explain',
      icon: 'bulb-outline',
      prompt: 'Explain this to me in simple terms.',
    },
        ...
        <View style={styles.inputRow}>
          ...
          >
            ...
            <TextInput
              ...
              placeholder={isRecording ? 'Listening...' : placeholder || 'Message Dash...'}
              ...
              onContentSizeChange={
                Platform.OS === 'web'
                  ...
                  : (e) => {
                      ...
                      setInputHeight((prev) => {
                        const nextHeight =
                          measuredHeight <= FULL_CHAT_GROW_THRESHOLD
                            ? FULL_CHAT_COMPACT_HEIGHT
